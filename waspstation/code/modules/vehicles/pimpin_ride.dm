...
/obj/vehicle/ridden/lawnmower
	...
	icon = 'waspstation/icons/obj/vehicles.dmi'
	icon_state = "lawnmower"
	var/emagged = FALSE
	var/list/drive_sounds = list('waspstation/sound/effects/mowermove1.ogg', 'waspstation/sound/effects/mowermove2.ogg')
	var/list/gib_sounds = list('waspstation/sound/effects/mowermovesquish.ogg')
	var/driver
...
/obj/vehicle/ridden/lawnmower/Move()
	...
	if(emagged)
		for(var/mob/living/carbon/human/M in loc)
			if(M == H)
				continue
			if(!(crossed.mobility_flags & MOBILITY_STAND)) 
