//Command
/datum/job/captain
	alt_titles = list("Overseer", "Station Manager")
	senior_title = "Veteran Captain"

/datum/job/head_of_personnel
	alt_titles = list("Human Resources Officer")
	senior_title = "Senior Bureaucrat"

/datum/job/lieutenant
	alt_titles = list("Bridge Officer", "Command Assistant")
	senior_title = "Senior Officer"

//Engineering
/datum/job/chief_engineer
	alt_titles = list("Engineering Coordinator")

/datum/job/engineer
	alt_titles = list("Maintenance Technician", "Engine Technician", "Electrician", "Junior Scientist")
	senior_title = "Senior Engineer"

/datum/job/atmos
	alt_titles = list("Firefighter", "Life Support Specialist")
	senior_title = "Senior Atmospheric Technician"

//Service
/datum/job/assistant
	alt_titles = list("Civilian", "Visitor", "Businessman", "Trader", "Entertainer")
	senior_title = "Greytider"

/datum/job/cook
	alt_titles = list("Cook", "Culinary Artist", "Butcher")
	senior_title = "Chef"

/datum/job/bartender
	alt_titles = list("Barkeep", "Server", "Mixologist")
	senior_title = "Master Mixologist"

/datum/job/hydro
	alt_titles = list("Gardener", "Herbalist", "Botanical Scientist", "Hydroponicist")
	senior_title = "Master Gardener"

/datum/job/curator
	alt_titles = list("Journalist", "Librarian")
	senior_title = "Sage"

/datum/job/janitor
	alt_titles = list("Custodian", "Sanitation Technician")
	senior_title = "Master Janitor"

/datum/job/lawyer
	alt_titles = list("Corporate Representative", "Attorney")
	senior_title = "Ace Attorney"

/datum/job/chaplain
	alt_titles = list("Counselor")
	senior_title = "Fanatic"

/datum/job/clown
	alt_titles = list("Jester", "Comedian")
	senior_title = "Master Honker"

/datum/job/mime
	alt_titles = list("Performer")
	senior_title = "Master Mime"

//Science
/datum/job/rd
	alt_titles = list("Research Supervisor", "Head Scientist", "Chief Researcher")

/datum/job/scientist
	alt_titles = list("Xenobiologist", "Chemical Researcher", "Toxologist", "Junior Scientist")
	senior_title = "Senior Scientist"

/datum/job/roboticist
	alt_titles = list("Biomechanical Engineer", "Mechatronic Engineer", "Tech-Priest", "Bionicles Engineer")
	senior_title = "Senior Roboticist"

//Medical
/datum/job/cmo
	alt_titles = list("Medical Director", "Surgeon-General")

/datum/job/doctor
	alt_titles = list("Nurse", "Surgeon", "Junior Doctor", "Psychiatrist")
	senior_title = "Senior Medical Doctor"

/datum/job/paramedic
	alt_titles = list("Emergency Medical Technician", "Medic")

/datum/job/chemist
	alt_titles = list("Pharmacist", "Pharmacologist", "Junior Chemist")
	senior_title = "Senior Chemist"

/datum/job/geneticist
	alt_titles = list("Biochemist")

/datum/job/virologist
	alt_titles = list("Biochemist")

//Security
/datum/job/hos
	alt_titles = list("Security Commander", "Chief of Security")

/datum/job/warden
	alt_titles = list("Brig Chief")

/datum/job/detective
	alt_titles = list("Forensics Technician", "Private Investigator")
	senior_title = "Hard-boiled Detective"

/datum/job/brig_phys
	alt_titles = list("Security Medic")

/datum/job/officer
	alt_titles = list("Security Guard", "Junior Officer", "Prison Guard")
	senior_title = "Senior Officer"

/datum/job/prisoner
	alt_titles = list("Protected Custody")

//Supply

/datum/job/qm
	alt_titles = list("Supply Chief")

/datum/job/mining
	alt_titles = list("Explorer")
	senior_title = "Master Miner"

/datum/job/cargo_tech
	alt_titles = list("Mailroom Technician", "Deliveries Officer")
	senior_title = "Bounty Hunter"
