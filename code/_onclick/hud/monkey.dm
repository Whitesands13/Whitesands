...
/datum/hud/monkey/persistent_inventory_update()
	...
	if(hud_shown)
		if(M.back)
			M.back.screen_loc = ui_layout["ui_monkey_back"]
			M.client.screen += M.back
		if(M.wear_mask)
			M.wear_mask.screen_loc = ui_layout["ui_monkey_mask"]
			M.client.screen += M.wear_mask
		if(M.wear_neck)
			M.wear_neck.screen_loc = ui_layout["ui_monkey_neck"]
			M.client.screen += M.wear_neck
		if(M.head)
			M.head.screen_loc = ui_layout["ui_monkey_head"]
