/obj/item/clothing/under/plasmaman/engineering
	name = "engineering plasma envirosuit"
	desc = "An air-tight suit designed to be used by plasmamen exployed as engineers, the usual purple stripes being replaced by engineer's orange. It protects the user from fire and acid damage."
	icon_state = "engineer_envirosuit"
	item_state = "engineer_envirosuit"
	armor = list("melee" = 0, "bullet" = 0, "laser" = 0, "energy" = 0, "bomb" = 0, "bio" = 100, "rad" = 10, "fire" = 95, "acid" = 95)

/obj/item/clothing/under/plasmaman/atmospherics
	name = "atmospherics plasma envirosuit"
	desc = "An air-tight suit designed to be used by plasmamen exployed as atmos technicians, the usual purple stripes being replaced by atmos's blue."
	icon_state = "atmos_envirosuit"
	item_state = "atmos_envirosuit"

/obj/item/clothing/under/plasmaman/engineering/ce //WS edit plasmaman customization
	name = "chief engineers envirosuit"
	desc = "An airtight envirosuit designed to be used by plasmamen selected to be engine overseers."
	icon_state = "ce_envirosuit"
	item_state = "ce_envirosuit"

/obj/item/clothing/under/plasmaman/engineering/skirt //WS edit plasmaman customization
	name = "engineering plasma envirokilt"
	desc = "An air-tight suit designed to be used by the feminine plasmamen exployed as engineers, the usual purple stripes being replaced by orange, protecting the user from fire and acid damage."
	icon_state = "engineer_enviroskirt"
	item_state = "engineer_enviroskirt"

/obj/item/clothing/under/plasmaman/atmospherics/skirt //WS edit plasmaman customization
	name = "atmospherics plasma envirokilt"
	desc = "An air-tight suit designed to be used by the feminine plasmamen exployed as atmos technicians, the usual purple stripes being replaced by atmos's blue."
	icon_state = "atmos_enviroskirt"
	item_state = "atmos_enviroskirt"

/obj/item/clothing/under/plasmaman/engineering/ce/skirt //WS edit plasmaman customization
	name = "chief engineers envirokilt"
	desc = "An airtight envirosuit designed to be used by the more feminine plasmamen selected to be engine overseers."
	icon_state = "ce_enviroskirt"
	item_state = "ce_enviroskirt"
