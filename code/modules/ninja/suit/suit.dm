...
/**
  * WS edits Proc for changing the suit's appearance upon locking.
  *
  * Proc for when space ninja's suit locks.  Gives it glowing lights, along with having an alternate sprite for female body types.
  * Arguments:
...
/**
  * WS edits Proc called to lock the important gear pieces onto space ninja's body.
  *
  * Called during the suit startup to lock all gear pieces onto space ninja.
  * Terminates if a gear piece is not being worn.  Also gives the ninja the inability to use firearms. Cringe.
  * If the person in the suit isn't a ninja when this is called, this proc just blows them instead.
  * Arguments:
  * * ninja - The person wearing the suit.
  */
...
/**
  * WS edits Proc called to unlock all the gear off space ninja's body.
  *
  * Proc which is essentially the opposite of lock_suit.  Lets you take off all the suit parts.
  * Also gets rid of the objection to using firearms from the wearer. If you want to use guns, you have to turn off the ninja suit.
  * Arguments:
  * * ninja - The person wearing the suit.
  */
