...
/**
  * WS edit Proc called to activate space ninja's adrenaline.
  *
  * Proc called to use space ninja's adrenaline.  Gets the ninja out of almost any stun.
  * After a bit, it injects the user with
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjaboost()
	if(!ninjacost(0,N_ADRENALINE))
		...
		a_boost = FALSE
		to_chat(ninja, "<span class='notice'>You have used the stimulant boosters.</span>")
		s_coold = 6
		addtimer(CALLBACK(src, .proc/ninjaboost_after), 70)

/**
  * WS edit Proc called to inject the ninja with radium.
  *
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjaboost_after()
	...
	to_chat(ninja, "<span class='danger'>You start feeling a metallic sense of vertigo, the start of the stimulant injection's after-effects.</span>")
