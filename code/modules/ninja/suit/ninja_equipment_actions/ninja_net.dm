...
/**
  * WS edits Proc called to create an energy net.
  *
  * Used to ensnare a target in an energy net, keeping them in place until the net is broken.
  * Costs 80E, which is 80% of the default battery's max charge. Is awkward to activate, expensive, and intended as a disabling tool against solo opponents.
  */
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjanet()
	...
	if(!ninjacost(800,N_STEALTH_CANCEL))
		...
		net.buckle_mob(net_target, TRUE) //fucklecuff vibes
