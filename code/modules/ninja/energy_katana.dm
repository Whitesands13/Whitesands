/**
  * # WS edits Energy Katana
  *
  * The katana that only the space ninja gets.  Comes with 30 force and a special jaunting system.
  * Upon clicking on a tile with the dash on, the user will teleport to that tile, assuming their target was not dense.
  * The katana has 2 dashes stored at maximum, and upon using the dash, it will recharge 20 seconds after it was used.
  * It also has a special feature where if it is thrown at the space ninja who owns it (determined by the ninja suit's serial number), the ninja will automatically catch the katana instead of being hit by it, even if throw mode is off for the ninja.
  *
  */
